...
struct UniformValue {
  ...
  std::uniform_real_distribution<double>
      dist; ///< Interval and distribution type

  /// @brief Constructs an instance producing random values uniformly
  ...
  UniformValue(const double min, const double max, const std::size_t seed = 1)
      : algo(seed), dist(std::min(min, max), std::max(min, max)) {}
...
struct UniformCircle {
  ...
  std::mt19937 algo; ///< Random number generator
  std::uniform_real_distribution<double>
      x; ///< Interval and distribution type for the x-coordinate
  std::uniform_real_distribution<double>
      y; ///< Interval and distribution type for the y-coordinate
  std::uniform_real_distribution<double>
      r; ///< Interval and distribution type for the r-coordinate

  /// @brief Constructs an instance producing random circles uniformly
  /// distributed (center and radius)
  ...
  /// @param seed Seed used to initialize the generator algorithm
  UniformCircle(const Vec2d &c_min, const Vec2d &c_max, double r_min,
                double r_max, const std::size_t seed = 1)
      : algo(seed),
        x(std::min(c_min[0], c_max[0]), std::max(c_min[0], c_max[0])),
        y(std::min(c_min[1], c_max[1]), std::max(c_min[1], c_max[1])),
        r(std::min(r_min, r_max), std::max(r_min, r_max)) {}
...
struct UniformTriangle {
  ...
  std::uniform_real_distribution<double>
      c_r; ///< distribution for the radius of the circumcircle
  std::uniform_real_distribution<double>
  ...
  ///  - uniformly distributed corner points w.r.t. three fixed non-overlapping
  ///  120-degree segments on the unit circle
  ...
  /// @param seed Seed used to initialize the generator algorithm
  UniformTriangle(const Vec2d &c_min, const Vec2d &c_max, double r_min,
                  double r_max, const std::size_t seed = 1)
      : algo(seed),
        c_x(std::min(c_min[0], c_max[0]), std::max(c_min[0], c_max[0])),
        c_y(std::min(c_min[1], c_max[1]), std::max(c_min[1], c_max[1])),
        c_r(std::min(r_min, r_max), std::max(r_min, r_max)),
        angle120(0, std::numbers::pi * 2 / 3),
        angle360(0, std::numbers::pi * 2) {}

  /// @brief Obtains the next random triangle using current state of the
  /// generator
  /// @return Random circle
  Triangle operator()() {
    ...
    auto scale = [r](const Vec2d &p) -> const Vec2d {
      return {p[0] * r, p[1] * r};
    };
